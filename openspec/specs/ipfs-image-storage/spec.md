...
Pin uploaded card images to IPFS (Pinata in production, a local Kubo node in development), store the canonical `ipfs://<CID>` URI in Postgres instead of raw image bytes, resolve those URIs to fetchable gateway URLs at the API response boundary, and unpin replaced images.
