...
/**
 * Renders a wallet's public profile — header, stats, achievements and reviews.
 * Shared by the owner's `/profile` page and the public `/profile/[address]`
 * store view. The "Edit profile" action is only shown when `isOwner` is true.
 */
export function ProfileView({ address, isOwner }: { address: string; isOwner: boolean }) {
